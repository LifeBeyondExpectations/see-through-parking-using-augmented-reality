...
flag_1_subscribeImg_2_loadImgFile = 1
...
class dataLoadClass:
    ...
    def __init__(self, imgInst, calibInst=None):
        ...
        self.viewTransformInst = ViewTransform(flag=flag_1_subscribeImg_2_loadImgFile, cctvViewPath=path_cctv_homograph_image, dataLoadInst=self)
    ...
    def carInfo_Request(self, req):
        # print('serviceOutCarInfo(carInfo_Request) : ', (self.carLocation[0], self.carLocation[1]), self.carAngle)
        return carInfoResponse(0, 0, 0, tuple(self.homographyInst.homography.reshape(9)), self.imgInst.width, self.imgInst.height)
    ...
    def callback(self, data):
        ...
        try:
            ...
            if flag_is_compressed_image == 0:
                ...
                self.imgInst.saveImage(self.bridge.imgmsg_to_cv2(data, "bgr8"), resize=(1288, 964)) #(640, 488)

            elif flag_is_compressed_image == 1:
                np_arr = np.fromstring(data.data, np.uint8)
                self.imgInst.saveImage(cv2.imdecode(np_arr, cv2.IMREAD_COLOR), resize=(1288, 964))  # cv2.CV_LOAD_IMAGE_COLOR is out of version
    ...
    def wrapper(self, nameOfFile=None):
        ...
        # homography
        if flag_undistort_to_homography == 1:
            self.imgInst.imgHomography = self.calibInst.startUndistort(self.imgInst.imgUndistort)
        else:
            self.imgInst.imgHomography = self.imgInst.imgUndistort

        # cv2.imshow('carView :self.imgInst.imgHomography', cv2.resize(self.imgInst.imgHomography, (0,0), fx=0.3, fy=0.3))
        # cv2.waitKey(1)

        # warp
        if flag_homography_to_warpAffine == 1:
            # self.carLocation[0], self.carLocation[1], self.carAngle = self.viewTransformInst.getCarLocationAngle()
            # print(self.carLocation[0], self.carLocation[1], self.carAngle)
        ...
        if flag_saveImg == 1 and flag_1_subscribeImg_2_loadImgFile == 2:
            ...
            cv2.imshow('imgWarp', self.imgInst.imgWarp)
            # cv2.imshow('self.imgInst.imgWarp', np.concatenate((self.imgInst.imgHomography ,self.imgInst.imgWarp), axis=1))
            cv2.waitKey(1)
    ...
    try:
        if flag_1_subscribeImg_2_loadImgFile == 1:
            ...
            if flag_publishImg == 1:
                ...
                dataLoadInst.serviceOutCarInfo()
                # while(True):
